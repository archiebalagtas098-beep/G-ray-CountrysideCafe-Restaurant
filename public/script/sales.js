...
async function loadSalesReportByPeriod(period) {
    try {
        ...
        salesData.margin = 30; // Fixed margin at 30%
        ...
        <body>
            <div class="container">
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>${formatCurrency(reportData.summary.totalRevenue * 0.70)}</td>
                        </tr>
                        <tr>
                            <td>Gross Profit (30%)</td>
                ...
                <div class="footer">
                    ...
                    <p>© Graycountryside Cafe 2026. All rights reserved.</p>
                </div>
            </div>
        </body>
        </html>
...
async function exportSalesReport(format = 'pdf') {
    ...
    try {
        ...
        // Get all orders for today to calculate breakdown
        const phTime = new Date();
        const startDate = new Date(phTime.getFullYear(), phTime.getMonth(), phTime.getDate());
        startDate.setHours(0, 0, 0, 0);
        const endDate = new Date(startDate.getTime() + 24 * 60 * 60 * 1000);
        
        // Fetch all today's orders from backend for accurate breakdown
        const response = await authenticatedFetch('/api/orders?status=completed&limit=1000');
        if (response.ok) {
            const result = await response.json();
            const allOrders = Array.isArray(result) ? result : (result.data || result.orders || []);
            
            // Filter today's completed orders and calculate breakdown
            allOrders.forEach(order => {
                const orderDate = new Date(order.createdAt);
                if (orderDate >= startDate && orderDate < endDate && 
                    (order.status === 'completed' || order.status === 'Completed' || order.status === 'paid' || order.status === 'Paid')) {
                    
                    if (order.items && Array.isArray(order.items)) {
                        order.items.forEach(item => {
                            const itemName = item.name || item.productName || 'Unknown';
                            let category = getItemCategory(itemName);
            ...
            <body>
                ...
                <div class="financial-summary">
                    ...
                    <div class="financial-row">
                        ...
                        <span>${formatCurrency(reportData.summary.totalRevenue * 0.70)}</span>
                    </div>
                ...
                <div class="footer">
                    ...
                    <p>Report ID: ${dateStr}-${timeStr}</p>
                    <p>&copy; Graycountryside Cafe 2026. All rights reserved.</p>
                </div>       
                <script>
                ...
                <div class="container">
                    ...
                    <table>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>Cost of Goods (70%)</td>
                                <td style="text-align: right;">${formatCurrency(reportData.summary.totalRevenue * 0.70)}</td>
                            </tr>
                            <tr class="total-row">
                                <td>Gross Profit (30%)</td>
...
function exportToExcel(reportData, dateStr, timeStr) {
    try {
        ...
        csvContent += `Total Revenue,${reportData.summary.totalRevenue}\n`;
        csvContent += `Cost of Goods,${reportData.summary.totalRevenue * 0.35}\n`;
        csvContent += `Gross Profit,${reportData.summary.grossProfit}\n\n`;
        
        // Add Revenue Breakdown section
        if (reportData.revenueBreakdown && Object.keys(reportData.revenueBreakdown).length > 0) {
            csvContent += "REVENUE BREAKDOWN BY CATEGORY\n";
            ...
            Object.entries(reportData.revenueBreakdown).forEach(([category, data]) => {
                ...
                const percentage = reportData.summary.totalRevenue > 0 && !isNaN(amount) && amount > 0
                    ? ((amount / reportData.summary.totalRevenue) * 100).toFixed(1)
                    : '0.0';
                csvContent += `${category},${amount.toFixed(2)},${percentage}%\n`;
            });
            csvContent += "\n";
        }
        ...
        // Add footer
        csvContent += "\n";
        csvContent += "Generated by Gray Countryside Cafe POS System\n";
        csvContent += `Report ID: ${dateStr}-${timeStr}\n`;
        csvContent += "© Graycountryside Cafe 2026. All rights reserved.\n";
...
function updateRevenueBreakdown() {
    ...
    if (currentPeriod === 'daily') {
        dateStr = today.toLocaleDateString('en-PH', { month: 'short', day: 'numeric', year: 'numeric' });
        periodLabel = 'Today';
        ...
        dateStr = `Weekly`;
        periodLabel = 'Once Per Week';
    } else if (currentPeriod === 'monthly') {
        // Monthly = specific date (1st to end of month)
        const monthStart = new Date(today.getFullYear(), today.getMonth(), 1);
        const monthEnd = new Date(today.getFullYear(), today.getMonth() + 1, 0); // Last day of month
        dateStr = `${monthStart.toLocaleDateString('en-PH', { month: 'short', day: 'numeric' })} - ${monthEnd.toLocaleDateString('en-PH', { month: 'short', day: 'numeric', year: 'numeric' })}`;
